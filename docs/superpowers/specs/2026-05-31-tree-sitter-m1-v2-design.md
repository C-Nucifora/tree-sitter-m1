...
> placeholder**. None is drawn from any real MoTeC m1-example project. The repo
...
m1-example corpus (all 80 scripts parse with zero ERROR/MISSING nodes), a generated
...
- **`scripts/check-corpus.sh`** — acceptance gate: parses every `*.m1scr` under
  the m1-example Scripts dir, fails on any ERROR/MISSING node.
...
- **Acceptance gate** (`scripts/check-corpus.sh`): must still report
  `0 with ERROR/MISSING nodes` over the m1-example corpus after the scanner change
...
| Item | Reason |
|------|--------|
| `xor` / `mod` word-operators | No occurrence in the m1-example corpus; unconfirmed against the M1 Development Manual. Adding reserved words now risks breaking real names. |
