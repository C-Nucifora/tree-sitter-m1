...
**Goal:** Add a standalone `interpolation` node for operand-position `$(VAR)`, a corrected and enriched `highlights.scm`, a new `locals.scm`, two new Rust query constants, and a broadened corpus (interpolation, scanner edges, error recovery, literals) — without regressing the EV-M1 acceptance gate.
...
- [ ] **Step 3:** confirm the acceptance gate is green (skip gracefully if the EV-M1 corpus is absent):

```bash
[ -d ../EV-M1/UQR-EV/01.00/Scripts ] && bash scripts/check-corpus.sh || echo "corpus absent — gate deferred to Task 6"
```
...
## Task 6: Acceptance gate — EV-M1 corpus must stay clean
...
```bash
[ -d ../EV-M1/UQR-EV/01.00/Scripts ] && bash scripts/check-corpus.sh || echo "corpus absent — gate skipped"
```
...
- [ ] **Step 4:** sanity-check no real EV-M1 content leaked into the docs:

```bash
grep -rniE 'EV-M1|UQR' README.md PLAN.md STATUS.md docs/ | grep -viE 'synthetic|placeholder|corpus|sibling|M1_CORPUS|example project'
```
...
```bash
[ -d ../EV-M1/UQR-EV/01.00/Scripts ] && bash scripts/check-corpus.sh || echo "corpus absent"
```
...
| 6 | Acceptance gate (EV-M1 stays clean) | — (verification) |
...
| Item | Reason |
|------|--------|
| `xor` / `mod` word-operators | No occurrence in the EV-M1 corpus; unconfirmed against the M1 Development Manual; adding reserved words risks breaking real multi-word names. |
